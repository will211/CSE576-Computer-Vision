...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        for (int k = 0; k < bn; k++) {
            d = l1_distance(a[j].data, b[k].data, a[j].n);

            if (d < min_distance) {
                min_distance = d;
                m[j].distance = d; // <- should be the smallest L1 distance!
                m[j].bi = k;
                m[j].q = b[k].p;
            }
        }
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
    ...
    for (i = 0; i < n; i++) {
        point projected_p = project_point(H, m[i].p);
        if (point_distance(m[i].q, projected_p) < thresh) {
            if (i != count) {
                match temp = m[i];
                m[i] = m[count];
                m[count] = temp;
...
image cylindrical_project(image im, float f)
{
    ...
    for (int i = 0; i < im.w; i++) {
        for (int j = 0; j < im.h; j++) {
            ...
            float x_pp = f * (x_p / z_p) + xc;
            float y_pp = f * (y_p / z_p) + yc;

            if (x_pp >= 0 && y_pp >= 0 && x_pp < im.w && y_pp < im.h) {
                for (int k = 0; k < im.c; k++) {
                    set_pixel(c, i, j, k, bilinear_interpolate(im, x_pp, y_pp, k));
                }
            }
        }
    }
